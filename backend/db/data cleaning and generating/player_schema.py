...
for index, row in all_player_df.iterrows():
  ...
  try:
    ...
    if idx != -1:
      ...
      output_string += "\t("+row['playerId']+", '"+new_name+"', '"+row['birthDate']+"', "+row['weight'][0:3]+", "+str(inch_to_cm(row['height']))+", '"+row['team']+"', 'https://cms.nhl.bamgrid.com/images/headshots/current/168x168/skater.jpg', "+str(2023-int(row['birthDate'][0:4]))+", '"+row['position']+"'),\n"
    else:
      output_string += "\t("+row['playerId']+", '"+row['name']+"', '"+row['birthDate']+"', "+row['weight'][0:3]+", "+str(inch_to_cm(row['height']))+", '"+row['team']+"', 'https://cms.nhl.bamgrid.com/images/headshots/current/168x168/skater.jpg', "+str(2023-int(row['birthDate'][0:4]))+", '"+row['position']+"'),\n"
  else:
    if idx != -1:
      new_name = row['name'][:idx]+"'"+row['name'][idx:]
      output_string += "\t("+row['playerId']+", '"+new_name+"', '"+row['birthDate']+"', "+row['weight'][0:3]+", "+str(inch_to_cm(row['height']))+", '"+row['team']+"', '"+url+"', "+str(2023-int(row['birthDate'][0:4]))+", '"+row['position']+"'),\n"
    else:
      output_string += "\t("+row['playerId']+", '"+row['name']+"', '"+row['birthDate']+"', "+row['weight'][0:3]+", "+str(inch_to_cm(row['height']))+", '"+row['team']+"', '"+url+"', "+str(2023-int(row['birthDate'][0:4]))+", '"+row['position']+"'),\n"
# output the string
